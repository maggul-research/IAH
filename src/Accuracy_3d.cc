...
namespace MyNSE
{
                                   ...
                                   Vector<double>   &values) const
  {
    ...
    values[0] = aa * std::pow(y, 2);
    values[1] = 0.0;
    values[2] = aa * std::pow(x, 3);
    values[3] = aa * 2.0 * Pr * x;
    values[4] = (3.0 * std::pow(aa, 2) * std::pow(x, 2) * std::pow(y, 2) -
                 6.0 * Pr * aa * x) /
                (Pr * Ra);
  }
                                   ...
                                   Vector<double>   &values) const
  {
    ...
    values[3] = 0.0;
    values[4] =
      (-6.0 * std::pow(aa, 2) / (Pr * Ra)) * (std::pow(x, 2) + std::pow(y, 2)) +
      (6.0 * std::pow(aa, 3) * x * std::pow(y, 4) -
       6.0 * Pr * std::pow(aa, 2) * std::pow(y, 2)) /
        (Pr * Ra);
  }
    ...
    std::vector<Tensor<1, dim>> &gradient_values) const
  {
    ...
    gradient_values[0][1] = aa * 2.0 * y;
    gradient_values[0][2] = 0.0;
    gradient_values[1][0] = 0.0;
    gradient_values[1][1] = 0.0;
    gradient_values[1][2] = 0.0;
    gradient_values[2][0] = aa * 3.0 * std::pow(x, 2);
    gradient_values[2][1] = 0.0;
    gradient_values[2][2] = 0.0;
  }
                                          ...
                                          const unsigned int) const
  {
    ...
    return_value[0] =
      (6.0 * std::pow(aa, 2) * x * std::pow(y, 2) - 6.0 * Pr * aa) / (Pr * Ra);
    return_value[1] = (6.0 * std::pow(aa, 2) * y * std::pow(x, 2)) / (Pr * Ra);
  ...
  NaturalConvection<dim>::solve(int solve_id)
  {
    ...
    if (solver_type == 0)
      {
        SolverControl               solver_control(100000, 1e-7, true);
  ...
  try
    {
      ...
      NaturalConvection<3> flow(alg, degree, Pr, Ra, rhoU, rhoT, alpha, beta);
      flow.run(n_ref);
    }
